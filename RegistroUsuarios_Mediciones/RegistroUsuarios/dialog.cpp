    ...
    ui(new Ui::Dialog)
{
    ...
    mDataBase->setDatabaseName("usuarios");
    ...
    ui->sensor->backgroundColourButton->setStyleSheet("background-color: red");
...
void Dialog::on_eliminar_clicked()
{
    mModel->removeRow(ui->tableView->currentIndex().row());
}

void Dialog::on_actualizar_clicked()
{
    mModel->select();

}

void Dialog::on_sensor_clicked()
{
    QSqlDatabase dBase;
    ...
    while(ki <= 10000){
        ...
        if( sg >= 10000){//paso un minuto
            ...
            for( int i = 0; i < 6; i++){
                ...
                ui->sensor->backgroundColourButton->setStyleSheet("background-color: green");
            }
        }
